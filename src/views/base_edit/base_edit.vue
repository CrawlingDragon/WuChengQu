...
export default {
  ...
  methods: {
    ...
    getEditData() {
      // 获取整条农事数据
      this.$axios
        .fetchPost("Mobile/Farmer/getFarmerDetail", {
          token: this.token,
          id: this.id
        })
        .then(res => {
    ...
    issueFramData() {
      ...
      if (this.id) {
        ...
        sucessMessage = "修改成功";
        url = "Mobile/Farmer/editFarmerData";
      } else {
        // 发布农事接口参数
        options = {
          type_desc: this.type_desc,
        ...
        sucessMessage = "发布成功";
        url = "Mobile/Farmer/addFarmerData";
      }
    ...
    getDrugChooseCase() {
      //获取用药的方案内容数组
      this.$axios
        .fetchPost("Mobile/Farmer/getProduct", { token: this.token })
        .then(res => {
          let data = res.data;
          if (data.code === 0) {
