    ...
    <div class="registration-box">
      ...
      <div class="item-box">
        <div class="item" v-for="item in list" :key="item.week">
          <div class=" left">
            {{ item.week }} <br />
            <span class="days">{{ item.ymd }}</span>
          </div>
          <div class="mid">
            <div
              class="p1"
              v-if="item.data.am.realname == '无号源'"
              @click="clickNo"
            >
              无号源
            </div>
            ...
            <div
              class="number"
              v-if="
                item.data.am.realname != '无号源' &&
                  item.data.am.realname != '即将放号'
              "
              @click="registration(item.data.am, '上午')"
            >
              总{{ item.data.am.nums }} 剩{{ item.data.am.surplus }}
            </div>
            <div class="expert" v-if="item.data.am.realname != '无号源'">
              {{ item.data.am.realname }}
            </div>
          </div>
          <div class="right">
            <div
              class="p1"
              v-if="item.data.pm.realname == '无号源'"
              @click="clickNo"
            >
              无号源
            </div>
            ...
            <div
              class="number"
              v-if="
                item.data.pm.realname != '无号源' &&
                  item.data.pm.realname != '即将放号'
              "
              @click="registration(item.data.pm, '下午')"
            >
              总{{ item.data.pm.nums }} 剩{{ item.data.pm.surplus }}
            </div>
            <div class=" expert" v-if="item.data.pm.realname != '无号源'">
              {{ item.data.pm.realname }}
            </div>
          </div>
        </div>
